...
PATTERN_BONUSES = {
    ("AK-47 | Case Hardened", 661): 100000, 
    ("AK-47 | Case Hardened", 321): 5000,   
    ("Karambit | Doppler", 563): 20000     
}
...
class DigitalAssetSkin:
    def __init__(self, skin_name, case_name, first_owner, float_value, paint_seed, stickers=None):
        self.name = skin_name
    ...
    def estimate_price(self):
        ...
        return price_with_float + pattern_bonus + sticker_bonus
    ...
    def display_full_details(self):
        ...
        if self.stickers:
            ...
            print("Adesivos Aplicados: Nenhum")
        self.display_provenance_tree()
        self.display_trade_history()
        
    def display_provenance_tree(self):
        title = "🌳 Árvore de Proveniência"
        print("\n" + "="*len(title)); print(title); print("="*len(title))
        def print_branch(node, prefix="", is_last=True):
            marker = "🔫" if node.type == "Skin" else "📦"
            print(prefix + ("└── " if is_last else "├── ") + f"{marker} [{node.type}] {node.name}")
...
def display_skin_list(skin_dict):
    if not skin_dict:
        ...
        print("-" * 75)
        for skin_id, skin in skin_dict.items():
            current_owner = skin.trade_history[-1].owner
            print(f"{skin_id:<4} | {skin.float_value:<10.6f} | {skin.paint_seed:<8} | {current_owner:<18} | {skin.name}")
...
def inspect_skin_by_id(database):
    try:
        skin_id = int(input("   Digite o ID da skin para inspecionar todos os detalhes: "))
        if skin_id in database:
            clear_console()
            database[skin_id].display_full_details()
        else:
            print("\n[ERRO] ID não encontrado no banco de dados.")
    except ValueError:
...
def trade_skin_by_id(database):
    try:
        skin_id = int(input("   Digite o ID da skin que deseja negociar: "))
        if skin_id in database:
            skin_to_trade = database[skin_id]
            current_owner = skin_to_trade.trade_history[-1].owner
            new_owner = input(f"   Para quem você quer enviar a '{skin_to_trade.name}'? (Dono Atual: {current_owner}): ")
            if new_owner and new_owner.lower() != current_owner.lower():
                skin_to_trade.trade(new_owner)
                print(f"\n[SUCESSO] Skin ID {skin_id} negociada com '{new_owner}'!")
            else:
                print("\n[ERRO] Nome do novo dono inválido ou é o mesmo do dono atual.")
        else:
...
def main_menu():
    ...
    while True:
        ...
        print("Use a lista ou a pesquisa para encontrar IDs e depois inspecione ou negocie.")
        
        print("\n--- MENU PRINCIPAL ---")
        print("1. 📜 Listar Todas as Skins no Database")
